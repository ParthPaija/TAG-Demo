...
{
    public class VIPLeaderboardManager : SerializedManager<VIPLeaderboardManager>
    {
        #region PUBLIC_VARIABLES

        public const int Max_Top_Rank = 10;
        ...
        public static bool isIntroViewShownRemianing = false;
        ...
        [ShowInInspector, SerializeField] private LeaderBoardRemoteConfigInfo myLeaderboardRCInfo;
        ...
        public bool IsEventResultRedayForShown()
        {
            bool isEventResult = !IsCurrentLeaderboardEventActive() && IsLastEventResultReadyToShow();

            if (IsCurrentLeaderboardEventActive() && !VIPLeagueManager.Instance.IsVIPLeageUnlocked() && Instance.IsLastEventResultReadyToShow())
            {
                isEventResult = true;
            }
        ...
        public bool IsEventForceComplateAndLastEventResultResdyToShown()
        {
            return !VIPLeagueManager.Instance.IsVIPLeageUnlocked() && IsLastEventResultReadyToShow();
        }
        ...
        {
            return PlayerPersistantData.GetVIPLeaderboardPlayerData().playerScore;
        }
        ...
        public void OnLeaderboardViewVisited()
        {
            ...
            //if (!IsCurrentLeaderboardEventActive() && !leaderBoardPlayerData.isEventResultShown)
            {
                ...
                if (playerRank <= Max_Top_Rank)
                {
                    ...
                    var currencyReward = rewardToGive.rewards.Find(x => x.GetRewardType() == RewardType.Curreny);
                ...
                if (!IsLeaderboardUnlocked())
                    PlayerPersistantData.SetVIPLeaderboardPlayerData(null);

                MainSceneUIManager.Instance.GetView<MainView>().LeaderboardUIButton.CheckForLeaderboardButton();
                MainSceneUIManager.Instance.GetView<MainView>().SetUI();
            }
        }

        public int GetPlayerRank()
        {
            if (PlayerPersistantData.GetVIPLeaderboardPlayerData() == null)
                return 0;
        ...
        public void AddAndUpdatePlayerScore(int score = 1)
        {
            var data = PlayerPersistantData.GetVIPLeaderboardPlayerData();
            data.playerScore += score;

            PlayerPersistantData.SetVIPLeaderboardPlayerData(data);
        }
        ...
        private void InitializeLeaderboardPlayers()
        {
            ...
            leaderBoardUserPlayer.Init(Leaderboard_Player_Name, null, null, LeaderboardPlayerType.UserPlayer);
            ...
            for (int i = 0; i < leaderBoardPlayerData.leaderBoardBotPlayerPersistantDatas.Count; i++)
            {
                ...
                leaderBoardBotPlayer.Init(
                    leaderboardData.botNamesList.GetDataAtIndex(leaderBoardPlayerData.leaderBoardBotPlayerPersistantDatas[i].botNameIndex),
                    leaderboardData.botAvtarSpriteList.GetDataAtIndex(leaderBoardPlayerData.leaderBoardBotPlayerPersistantDatas[i].botNameIndex),
                    leaderboardData.botFrameSpriteList.GetDataAtIndex(leaderBoardPlayerData.leaderBoardBotPlayerPersistantDatas[i].botNameIndex)
                    );
        ...
        private void InitializeLeaderboardManager()
        {
            ...
            if (!IsLeaderboardUnlocked())
            {
                if (leaderBoardPlayerData != null)
                {
                    if (IsEventForceComplateAndLastEventResultResdyToShown())
                    {
            ...
            if (leaderBoardPlayerData == null)
                leaderBoardPlayerData = new VIPLeaderBoardPlayerPersistantData();
            ...
            if (isStartNewEvent)
            {
                isIntroViewShownRemianing = true;
                AutoOpenPopupHandler.Instance.OnCheckForAutoOpenPopUps();
                Debug.Log("<= Starting New Leaderboard Event =>");
                StartNewLeaderboardEvent(leaderBoardPlayerData);
                PlayerPersistantData.SetVIPLeaderboardPlayerData(leaderBoardPlayerData);
                AnalyticsManager.Instance.LogGAEvent("VipLeagueEntered : " + leaderBoardPlayerData.playerTargetScore);
            }
        ...
        private void StartNewLeaderboardEvent(VIPLeaderBoardPlayerPersistantData leaderBoardPlayerData)
        {
            ...
            leaderBoardPlayerData.currentLevel = 1;
            leaderBoardPlayerData.lastPlayedLevelIndex = -1;
            leaderBoardPlayerData.playerTargetScore = CalculateAndGetTargetLevel();
            leaderBoardPlayerData.leaderBoardBotPlayerPersistantDatas = new List<VIPLeaderBoardBotPlayerPersistantData>();
            ...
            {
                VIPLeaderBoardBotPlayerPersistantData botData = new VIPLeaderBoardBotPlayerPersistantData();
        ...
        {
            Debug.Log(SerializeUtility.SerializeObject(PlayerPersistantData.GetVIPLeaderboardPlayerData()));
        }
    ...
    public class LeaderBoardProgressTracker
    {
        ...
        {
            var datas = VIPLeaderboardManager.Instance.GetLeaderboardPlayerUIDatas();
    ...
    public class VIPLeaderBoardPlayerPersistantData
    {
        ...
        [JsonProperty("lbbppd")] public List<VIPLeaderBoardBotPlayerPersistantData> leaderBoardBotPlayerPersistantDatas = new List<VIPLeaderBoardBotPlayerPersistantData>();
        [JsonProperty("viplcl")] public int currentLevel = 1;
        [JsonProperty("vipllpli")] public int lastPlayedLevelIndex = -1;
    }

    public class VIPLeaderBoardBotPlayerPersistantData
    {
