...
namespace Tag.HexaStack
{
    public class InternetManager : Manager<InternetManager>
    {
        ...
        public Sprite noInterNetSprite;
        ...
        private void OnNetConnectionResponse(bool isNetAvailable)
        {
            //if (isNetAvailable)
                OnLoadingDone();
            //else
            //GlobalUIManager.Instance.GetView<UserMessagePromptView>().Show(() => { CheckNetConnection(OnNetConnectionResponse); });
            //GlobalUIManager.Instance.GetView<UserMessagePromptView>().Show("No internet!?",
            //    UserPromptMessageConstants.NoInternetConnection, noInterNetSprite, "Retry", "", () => { CheckNetConnection(OnNetConnectionResponse); }, null);
        }
