...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        public bool isOnlyActiveChild = false;
        ...
        internal void InitialiseChildObjectsFromScene()
        {
            ...
            {
                childCount = _screensContainer.childCount;

                List<GameObject> activeChild = new List<GameObject>();

                for (int i = 0; i < childCount; i++)
                {
                    GameObject child = _screensContainer.transform.GetChild(i).gameObject;
                    if (child.activeSelf)
                        activeChild.Add(child);
                }

                ChildObjects = new GameObject[activeChild.Count];
                for (int i = 0; i < activeChild.Count; i++)
                {
                    ChildObjects[i] = activeChild[i];
